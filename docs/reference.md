...
The error context distinguishes `not-attempted` (the target writer was not entered), `failed` (the writer was attempted and explicitly reported no replacement), `unknown` (replacement was reported but durability, cleanup, or target verification is uncertain), `committed` (the writer and target postcondition succeeded), and `unchanged` (a no-op was processed). `failed` is an error-context state only; the persistent journal schema is unchanged. Subsequent journal failure does not downgrade a known committed target. `changed_targets` lists known target replacements in plan order, including those followed by an uncertain writer outcome, and `committed` is true exactly when that list is nonempty. It does not describe journal publication: an initial journal failure before target writes has `committed = false`, while a final journal failure after target writes retains `committed = true`. These fields contain no configuration values, conditions, content digests, or exception payloads.
